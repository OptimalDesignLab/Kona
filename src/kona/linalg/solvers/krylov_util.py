...
def eigenvaluesAndVectors(n, A, eig, E):
	if n<1: 
		raise ValueError('krylov.cpp (eigenvaluesAndVectors): matrix dimension must be greater than 0.')

	if (A.shape[0]  < n) or (A.shape[1]  < n):
		raise ValueError('krylov.cpp (eigenvaluesAndVectors): given matrix has fewer rows/columns than given dimension.')

	if (E.shape[0]  < n) or (E.shape[1]  < n):
		raise ValueError('krylov.cpp (eigenvaluesAndVectors): Eigenvector matrix has fewer rows/columns than given dimension.')
	...
	if info!=0:	
		raise ValueError('krylov.cpp (eigenvaluesAndVectors): LAPACK routine dsyev failed with info ='  + info)
...
def trustFunction(n, H, g, lambda2, radius):
...
def solveUnderdeterminedMinNorm(nrow, ncol, A, b, x):
	...
	PTmat = np.zeros(ncol, ncol)
	Qmat = np.zeros(nrow, nrow)
