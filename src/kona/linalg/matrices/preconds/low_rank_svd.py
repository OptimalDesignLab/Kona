...
class LowRankSVD(BaseHessian):
    ...
    fwd_factory : :class:`~kona.linalg.memory.VectorFactory`-like
    ...
    rev_factory : :class:`~kona.linalg.memory.VectorFactory`-like, optional
        Vectors produced by this factory must be valid input vector for the
    ...
    _allocated : bool
        Flag to check if memory has already been allocated.
    _transposed : bool
        Flag to control which product to return in the generalized product call.
    fwd_mat_vec, rev_mat_vec : function
        Matrix-vector forward and reverse production functions.
    fwd_mat_factory, rev_mat_factory : :class:`~kona.linalg.memory.VectorFactory`-like
        Vector factories associated with the forward and reverse matrix products.
    Q, P : list of :class:`~kona.linalg.vectors.common.KonaVector`-like
        List of subspace vectors generated by the Lancsoz bi- or tri-diagonalization.
    U, V : list of :class:`~kona.linalg.vectors.common.KonaVector`-like
        List of left and right singular vectors.
    ...
    def product(self, in_vec, out_vec):
        if not self._transposed:
            self.approx_fwd_prod(in_vec, out_vec)
        else:
            self.approx_rev_prod(in_vec, out_vec)
        self._transposed = False
