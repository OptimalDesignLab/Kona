...
class FLECS_RSQN(OptimizationAlgorithm):
    """
    A reduced-space quasi-Newton optimization algorithm for PDE-governed
    (in)equality constrained problems.

    This algorithm is a variation of the FLECS-RSNK algorithm, where the
    Lagrangian Hessian block of the KKT matrix is replaced with a limited
    memory SR1 approximation. However, products with the total constraint
    jacobian blocks are still computed using a 2nd order adjoint formulation.
    ...
    def __init__(self, primal_factory, state_factory, dual_factory, optns={}):
        ...
        super(FLECS_RSQN, self).__init__(
